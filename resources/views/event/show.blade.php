        ...
        <div class="row" id="app">
            ...
            <img src="{{$reply->user->avatar_path}}" alt="">
                {{$reply->user->name}}</a> Replied</p>
                                    ...
                                    @if(Auth::user())
                                    <p class="mb-3"> <span class="float-left btn btn-link" @click="$modal.show('reply{{$reply->id}}')">Reply</span> @endif @if($reply->discussionreplies->count()>0)
                                        <span class="float-left btn btn-link" @click="$modal.show('replyall{{$reply->id}}')">View All Replies</span> @endif
                                    </p>
                                </div>
                                <div>
                                    @include('event.modal') @include('event.reply')
                                </div>
                            </div>
                        </reply>
                        @include('event.allreplies') @endforeach
                    </div>
                </div>
                {!! $replies->render() !!}
            </div>
               ...
               <!-- Event Right Side-->
                <div class="single-event_right mb-3">
                    ...
                    <div class="mt-4 single-event_organize">
                        <h5><b>Organizer</b></h5>
                        <p><a href="/profile/{{$event->user->id}}"><span><img src="{{$event->user->avatar_path}}" alt=""></span><span> {{$event->user->name}}</span></a> @if(Auth::user())
                ...
                @if($guests->count()>0)
                <button class="btn btn-link mt-3" @click="$modal.show('guestModal')"><h5>View All Event Guests</h5></button> @include('event.guest')
                @endif
            </div>
        </div>
    </div>
